...
func (server *Server) handleQuery(ctx context.Context, request query.Request) (responses []interface{}, err error) {
	...
	for _, target := range request.Targets {
		...
		response, err = server.handleQueryRequest(ctx, target, request)
		timer.ObserveDuration()
		if err == nil {
			responses = append(responses, response)
		} else {
			queryFailure.WithLabelValues(server.Name, target.Type, target.Name).Add(1.0)
			break
		}
	}
...
func (server *Server) handleQueryRequest(ctx context.Context, target query.Target, request query.Request) (query.Response, error) {
	handler, ok := server.Handlers[target.Name]
